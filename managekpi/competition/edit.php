...
if(isset($_GET["id"]) && $_GET["id"] != ""){
    ...
    $edit_id = $_GET['id'];
    $sql = "SELECT * FROM $type WHERE comp_id=$edit_id AND userID=" .$_SESSION['UID'];
                ...
                <form action="edit-action.php" method="post" class=kpi-form enctype="multipart/form-data">
                    ...
                    <div class="formsection">
                        ...
                        <select size="1" id="level" name="level" style="width:100%;" required>
                            <option value="">&nbsp;</option>
                            <option <?php echo($level == 'Faculty') ? 'selected' : '';?>>Faculty</option>;
                            <option <?php echo($level == 'University') ? 'selected' : '';?>>University</option>;
                            <option <?php echo($level == 'National') ? 'selected' : '';?>>National</option>;
                            <option <?php echo($level == 'International') ? 'selected' : '';?>>International</option>;
                        </select>
                    </div>

                    <div class="formsection">
                        <label for="competition"><p class="indicate"><b>Competition Name</b></p></label>
                        <input type="text" name="competition" required value='<?= $name?>'></textarea>
                    </div>
